...
void v3_log(char *buf)    // never used
{
U8 size;
...
// Command parser
void v3_proc_message(union v3_message_UNION * pv3msgU)                            // Process the CMD from APP by Jason
{
...
bool v3_get_sum( union v3_message_UNION * pv3msgU)
{
	...
	for(i=0;i<pv3msgU->v3msg.len;i++)
		pv3msgU->v3msg.sum +=pv3msgU->v3_buf8[i];
...
void v3_state(void)
{
   ...
   // Temporary code to force cables connected until new cables are made.
	...
	if ((v3status.statusa & 0x07) == 0x06)
		v3status.batsoc = 100;
	else
		v3status.batsoc = max14676_GetBatteryCharge();
	...
	if (v3status.state < V3_STATE_CHRG) // See if PMIC CHRGIN active, change state globally on charge OR power button
	{
		...
		if (!(v3status.conn & v3PINSPBTN) && (v3_sm!=V3_SM_OFF))
			btn_count++;
		else
			btn_count = 0;
...
//SinOn(v3combo.inten);
   ...
   switch (v3_sm)
   {
      case  V3_SM_IDLE:
      {
    	  if (v3combo.awakesec)
    	  {
    		  //v3combo.awakesec--;  // decrement power down reporting timers when idle                          Disable time out shutdowm by Jason
    	  }
    	  else
	      {
    		  if( v3status.inithandle != V3_NO_HANDLE)
    			  v3_init();                               // set init handle and other init when v3combo.awakesec terminates
    		  if (v3combo.offsec)
    			  v3combo.offsec--;
	      }
      ...
      case V3_SM_CHARGE:
      {
    	  ...
    	  {
#if 1                                                                                                        // Disable shut down when unplugging the charger by Jason
    		  ledseq_set(LEDPOWEROFF,1);  // kick off LED
    		  fbseq_set(FBPOWEROFF,1);    // Feedback sequence
    		  led_sm = LED_SM_OFF;        // signal to LED state machine
    		  v3_sm = V3_SM_OFF;          // shutdown,                                                        Charger unpluged
#endif
    	  }
...
//LED STATE MACHINE
   switch (led_sm)
   {
      ...
      case LED_SM_ADV:
    	  ...
    	  if ( v3status.batsoc < 50) ledseq_set(LEDCHRG25,LEDFOREVER);
